...
const Header = () => {
  ...
  <header className='header'>
    ...
    <div className='header_mobile'>
      ...
      <img className='header_mobile_burger' src={burger} alt='mobile menu'onClick={handleBurgerClick}/>
    </div>

    {/* phone, nav, button */}
    <nav className={`header_nav_wrapper ${isOpen ? 'header_nav_wrapper--open' : ''}`}>
      <p>+7 (495) 79 986 79</p>
      ...
      <ul className='header_nav'>
        {navItems.map((navItem, index) => (
          <li className='header_nav_item' key={index}>
            <Scrollchor to={`#${navItem.href}`} animate={{offset: 20, duration: 1000}}>
              {navItem.menuItem}
             </Scrollchor>
          </li>
        ))}
      ...
      <a href='#faq'>
        <Button className='button button--md button--green' text='Как установить киоск' />
      </a>
    </nav>
